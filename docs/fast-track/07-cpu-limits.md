...
**File**: `crates/cgroup-tool/tests/cpu_test.rs`
...
Run it: `sudo -E cargo test -p cgroup-tool --test cpu_test`
...
sudo cargo run -p cgroup-tool -- create /sys/fs/cgroup/cpulimit
sudo cargo run -p cgroup-tool -- cpu-max /sys/fs/cgroup/cpulimit 50000
...
sudo cargo run -p cgroup-tool -- attach /sys/fs/cgroup/cpulimit $PID
top -p $PID  # Watch CPU% stay around 50

# Cleanup
kill $PID
sudo cargo run -p cgroup-tool -- delete /sys/fs/cgroup/cpulimit
```

## What just happened

`cpu.max` uses quota/period format. With "50000 100000", the process gets 50000 microseconds of CPU time per 100000μs period = 50%. The kernel throttles the process when it hits quota. This is how containers get CPU limits.
